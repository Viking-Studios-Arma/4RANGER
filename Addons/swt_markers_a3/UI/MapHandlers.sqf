    ...
    if (_shift and !_alt and !_ctrlKey and ((_this select 1) == 0)) then {
		...
		if (!_shift and !_ctrlKey and _alt and ((_this select 1) == 0)) then {
			...
			if (_shift and !_ctrlKey and _alt and ((_this select 1) == 0)) then {
				...
				if ((_this select 1) == 0) then {
					swt_mouseButtonDown = true;
                    private _markersPool = swt_markers_allMarkers;
                    if (!isNil "swt_markers_briefingMarkers" && {missionNamespace getVariable ["wmt_param_DisableBreifingMarkerMove", false]}) then {
                        _markersPool = swt_markers_allMarkers - swt_markers_briefingMarkers;
                    };
					{
						_pos = getMarkerPos _x;
						_pos = _ctrl ctrlMapWorldToScreen _pos;
...
swt_markers_MapKeyDown = {
    ...
    _alt = _this select 4;
	if !(isNil "swt_markers_line_params_world") then {
		if ((_alt || _shift) && !(_alt && _shift)) then { //xor
