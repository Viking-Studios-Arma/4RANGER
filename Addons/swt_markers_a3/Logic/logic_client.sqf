...
//Make dump of markers placed at briefing stage
[] spawn {
    sleep 1;
    swt_markers_briefingMarkers = +swt_markers_allMarkers;
};

[{CBA_missionTime > 0},{
    ["created", {
        if (missionNamespace getVariable ["wmt_Main_ModuleRunning", false]) then {
            _this call swt_fnc_handlePolylines;
        };
